...
HP33120_SetSignal('SQU',2.5);
HP33120_SetDCoff(1.25);
HP33120_SetFreq(400);
...
HP54622_SetTimeScale(2.8e-3);
%HP54622_SetVerticalRange(1,2.5,0.2) % Useful for some scopes where the knobs do not work properly!!!
[time,data]=HP54622_GetData;
%ploting the first row of data (channel 1) vs time
theory = linspace(5, 0, 100);
theory_time = linspace(0,1.25536e-3, 100);
figure(1);
hold on;

plot(time,data(:,1));
plot(time, data(:,2));
plot(theory_time, theory);
hold off;
